...
{
	var fireScreen = firebase.database().ref().child('lessons/type/').child(whatlesson).child(i);
  ...
  if (snapshot.child(DorQ).val() == "0") { //CHECK IF HESBER OR SHEELON 0 HESBER 1 SHEELON IF 0 TRUE
	  ...
	  $('#hesberD2').show();
		...
		for (var j=0;j<answerscount;j++){
			answersofhesber.innerText += snapshot.child('answers/'+j).val()
			answersofhesber.appendChild(br);
		}
